...
# 임의로 삭제할 단어 카운트 초기화
resetWords <- c('제주', '여행', '숙소', '해안', '전망', '시간')
result[names(result)%in%resetWords] <- 0

result
# wordcloud 생성
# 전체 데이터
library(wordcloud)
pal <- brewer.pal(8, 'pastel3')
wordcloud(names(result), freq=result, scale=c(3, 0.5), min.freq=2, max.words = 100, random.order=F, rot.per=.1, colors=pal)

# 지정된 관광지 데이터만 포함
wordcloud(names(result[names(result)%in%spots]), freq=result, scale=c(3, 0.5), min.freq=2, max.words = 100, random.order=F, rot.per=.1, colors=pal)
